...
def plot_prediction(data):
    ...
    alpha = 1
    ax.plot(
        data["TimeData"]["Latitude"],
        data["TimeData"]["Longitude"],
    ...
    for ensembles in data["Secondary Tracks"]:
        ax.plot(
            data["Secondary Tracks"][str(ensembles)]["Lat"],
...
def PredictionAni(PredData, saving):
    ...
    flight.append(
        np.array(
            PredData["TimeData"][["Latitude", "Longitude", "Altitude"]]
        ).transpose()
    )
    ...
    fontSize = 30
    ax.set_xlim3d(
        [min(PredData["TimeData"]["Latitude"]), max(PredData["TimeData"]["Latitude"])]
    )
    # ax.set_xlabel('Latitude',fontsize=fontSize)

    ax.set_ylim3d(
        [min(PredData["TimeData"]["Longitude"]), max(PredData["TimeData"]["Longitude"])]
    )
    # ax.set_ylabel('Longitude',fontsize=fontSize)

    ax.set_zlim3d(
        [min(PredData["TimeData"]["Altitude"]), max(PredData["TimeData"]["Altitude"])]
    )
    ...
    line_ani = animation.FuncAnimation(
        fig,
        update_lines,
        frames=max(data[0].shape[1], data[1].shape[1]),
        fargs=(data, lines),
        interval=0.1,
        blit=True,
    )
...
def plotStations(loc):
    if loc == "US":
        stationData = pd.read_csv(
            os.getcwd() + "\\StationList.txt", delimiter=" ", header=None
        )
    return stationData
...
def plotFlight(flightID, predTotal, predLow, predHigh, plotEnsem, plotType):
    ...
    for prediction in AllData:
        ID = (
            str(AllData[prediction]["Inputs"]["Status"])
            + ","
            + str(math.trunc(AllData[prediction]["Inputs"]["Altitude"]))
        )
        ...
        if predLow < int(prediction) < predHigh:
            if plotType == "3":
                traceNom = go.Scatter3d(
                    x=AllData[prediction]["TimeData"]["Latitude"],
                    y=AllData[prediction]["TimeData"]["Longitude"],
                    z=AllData[prediction]["TimeData"]["Altitude"],
                    mode="lines",
                    line=dict(color="blue", width=1),
                    name=str(AllData[prediction]["Inputs"]["Status"])
                    + ","
                    + str(math.trunc(AllData[prediction]["Inputs"]["Altitude"])),
                )
            ...
            if plotType == "2A":
                traceNom = go.Scatter(
                    x=AllData[prediction]["TimeData"]["Altitude"].index,
                    y=AllData[prediction]["TimeData"]["Altitude"],
                    mode="lines",
                    line=dict(color="blue", width=1),
                    name=str(AllData[prediction]["Inputs"]["Status"])
                    + ","
            ...
            if plotType == "2L":
                traceNom = go.Scatter(
                    x=AllData[prediction]["TimeData"]["Longitude"],
                    y=AllData[prediction]["TimeData"]["Latitude"],
                    mode="lines",
                    line=dict(color="blue", width=1),
                    name=str(AllData[prediction]["Inputs"]["Status"])
                    + ","
        ...
        if predLow < int(prediction) < predHigh:
            if plotEnsem:
                for track in AllData[prediction]["Secondary Tracks"]:
                    if plotType == "3":
                        traceEns = go.Scatter3d(
                            x=AllData[prediction]["Secondary Tracks"][track]["Lat"],
                            y=AllData[prediction]["Secondary Tracks"][track]["Lon"],
                            z=AllData[prediction]["Secondary Tracks"][track]["Alt"],
                            mode="lines",
                            line=dict(color="#1f77b4", width=0.2),
                        )
                        data.append(traceEns)
                    if plotType == "2A":
                        traceEns = go.Scatter(
                            x=AllData[prediction]["Secondary Tracks"][track][
                                "Alt"
                            ].index,
                            y=AllData[prediction]["Secondary Tracks"][track]["Alt"],
                            mode="lines",
                            line=dict(color="#1f77b4", width=0.2),
                        )
                        data.append(traceEns)
                    if plotType == "2L":
                        traceEns = go.Scatter(
                            x=AllData[prediction]["Secondary Tracks"][track]["Lon"],
                            y=AllData[prediction]["Secondary Tracks"][track]["Lat"],
                            mode="lines",
                            line=dict(color="#1f77b4", width=0.2),
                        )
                        data.append(traceEns)
    ...
    if plotType == "2A":
        traceFlightPath = go.Scatter(
            x=FlightPath["alt"].index,
            y=FlightPath["alt"],
            mode="lines",
            line=dict(color="black", width=3),
            text=FlightPath.index,
...
def get_args(argv, queryTime, TESTINGtimeDiff, UTCdiff):
    ...
    if help == 1:
        ...
        print(
            "            -update  (Update the weather file based on position - slow!)"
        )
        print(
            "            -aprs  (use the current APRS position as starting point - for real time!)"
        )
        print("            -zero  (simulate a zero pressure balloon)")
        print("            -r=radius (for zero pressure balloon, in meters)")
        print(
            "            -v=total volume of balloon (for zero pressure balloon, in meters^3)"
        )
        print(
            "            -hover=Altitude of neutral buoyant point (ZP balloon - although automatic now)"
        )
        print(
            "            -loss=percentage loss rate of helium through balloon (% per minute)"
        )
        ...
        print("example: ")
        print(
            "  ./balloon.py -payload=6.0 -balloon=1000 -parachute=6.0 -helium=1.5 -lat=42.0 -lon=-84.0"
        )
    ...
    if queryTime == "now":
        LaunchTime = (
            ...
            + datetime.timedelta(hours=TESTINGtimeDiff)
            + datetime.timedelta(hours=UTCdiff)
        )
    if queryTime != "now":
        LaunchTime = datetime.datetime.strptime(
            queryTime, "%Y-%m-%d %H:%M:%S"
        ) + datetime.timedelta(hours=UTCdiff)
...
def get_station(longitude, latitude):
    ...
    stationData[10] = (
        (stationData[3] - latitude) ** 2 + (stationData[6] - longitude) ** 2
    ) ** 0.5
    ...
    if MinDist > math.sqrt(500):
        stationDataWorld = pd.read_csv(
            os.getcwd() + "\\StationListWorld.txt", delimiter=" ", header=None
        )
        stationDataWorld[10] = (
            (stationDataWorld[3] - latitude) ** 2
            + (stationDataWorld[6] - longitude) ** 2
        ) ** 0.5
    ...
    if MinDist == DistSave:
        url = (
            "http://www.meteor.iastate.edu/~ckarsten/bufkit/data/nam/nam_"
            + stat
            + ".buf"
        )
...
def KaymontBalloonBurst(BalloonMass):
    # Balloon Masses
    kaymontMass = [200, 300, 350, 450, 500, 600, 700, 800, 1000, 1200, 1500, 2000, 3000]

    # Burst diameter in meters
    kaymontBurstDiameter = [
        3.00,
        3.78,
        4.12,
        4.72,
...
def calc_ascent_rate(RapData, NumberOfHelium, args, altitude):
    ...
    if NetLiftForce > 0.0:
        AscentRate = np.sqrt(
            2 * NetLiftForce * corrector / (BalloonDragCoefficient * Area * MassDensity)
        )
    else:
        AscentRate = -np.sqrt(
            -2
            * NetLiftForce
            * corrector
            / (BalloonDragCoefficient * Area * MassDensity)
        )
...
def calc_descent_rate(RapData, args, altitude):
    ...
    DescentRate = np.sqrt(
        ...
        * args["payload"]
        * Gravity
        / (MassDensity * ParachuteDragCoefficient * args["area"])
    )
...
def prediction(
    ...
    if args["balloon"] > 0:
        ...
        if BurstDiameter > 0:
            ...
            # Get weather station
            filename, IsNam, StatLat, StatLon, MinDist = get_station(
                longitude, latitude
            )
            StationLat = [StatLat]
            StationLon = [StatLon]
            ...
            while Diameter < BurstDiameter and altitude > -1.0:
                ...
                NumberOfHelium = NumberOfHelium * (
                    1.0 - args["loss"] / 100.0 / 60.0 * dt
                )

                Veast, Vnorth = get_wind(RapData, altitude)
                DegPerMeter = 360.0 / (2 * pi * (EarthRadius + altitude))
                longitude = longitude + Veast * DegPerMeter * dt / np.cos(
                    latitude * dtor
                )
                ...
                TotalDistance += (
                    np.sqrt(Veast * Veast + Vnorth * Vnorth) * dt * MilesPerMeter
                )

                AscentRate, Diameter = calc_ascent_rate(
                    RapData, NumberOfHelium, args, altitude
                )
            ...
            while i < nEnsembles:
                ...
                NumberOfHelium = calculate_helium(args["helium"])
                NumberOfHeliumPerturbed = random.normalvariate(
                    NumberOfHelium, NumberOfHelium * errors / 4
                )
                ...
                if status != -1:
                    ...
                    while Diameter < BurstDiameter and altitude > -1.0:
                        ...
                        AscentRate, Diameter = calc_ascent_rate(
                            RapData, NumberOfHeliumPerturbed, args, altitude
                        )
                        ### Reset ascentRate here? ###
                        if ARcorr != -1:
                            AscentRate = ARcorr
                    ...
                    DifferenceInPeakAltitude = (
                        DifferenceInPeakAltitude + (altitude - PeakAltitude) ** 2
                    )
                ...
                secondaryTracks[str(i)] = pandas.DataFrame(
                    index=pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                )
                ...
                if i == 0:
                    ...
                    secondaryTracksN["Lat"]["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                    ...
                    secondaryTracksN["Lon"]["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                    ...
                    secondaryTracksN["Alt"]["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                ...
                if i > 0:
                    ...
                    tempTrackLat["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                    ...
                    tempTrackLon["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                    ...
                    tempTrackAlt["time"] = pandas.to_datetime(
                        pandas.to_datetime(args["launchtime"])
                        + pandas.to_timedelta(TotalTimeEnsem, unit="s")
                    )
                    tempTrackAlt[str(i)] = np.array(secAlt) * 3.28084

                    secondaryTracksN["Lat"] = pandas.merge(
                        secondaryTracksN["Lat"],
                        tempTrackLat,
                        sort=True,
                        on="time",
                        how="outer",
                    )
                    secondaryTracksN["Lon"] = pandas.merge(
                        secondaryTracksN["Lon"],
                        tempTrackLon,
                        sort=True,
                        on="time",
                        how="outer",
                    )
                    secondaryTracksN["Alt"] = pandas.merge(
                        secondaryTracksN["Alt"],
                        tempTrackAlt,
                        sort=True,
                        on="time",
                        how="outer",
            ...
            if nEnsembles > 1:
                DifferenceInPeakAltitude = np.sqrt(
                    DifferenceInPeakAltitude / nEnsembles
                )
    ...
    AllData = dict()
    AllData["TimeData"] = pandas.DataFrame(
        index=pandas.to_datetime(
            pandas.to_datetime(args["launchtime"])
            + pandas.to_timedelta(TotalTime, unit="s")
        )
    )
